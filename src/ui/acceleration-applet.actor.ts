 ...
 * Normalizes by fullScale (the acceleration mapped to the boundary edge, symmetric on both
 * axes), clamps the vector magnitude to the boundary radius (so the dot stays on/inside the
 * circle along the true direction), and negates y so that speeding up (positive y) moves the
 * dot up. Positive x moves the dot right.
 */
export function calcDotOffset(acceleration: { x: number, y: number }, boundaryRadius: number, fullScale: number): { x: number, y: number } {
    let nx = acceleration.x / fullScale;
    let ny = acceleration.y / fullScale;
...
export class AccelerationAppletActor extends ScreenElement {
    ...
    private renderIndicator(ctx: CanvasRenderingContext2D): void {
        ...
        // fallback divisor only used before a vehicle is attached, when acceleration is zero anyway
        const fullScale = this.vehicle?.accelerationFullScale ?? 1;
        ...
        const offset = calcDotOffset(acceleration, this.boundaryRadius, fullScale);
